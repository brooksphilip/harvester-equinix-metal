...
#     command = "rsync --rsync-path=\"sudo rsync\" rancher@${local.ssh_ip}:/etc/rancher/rke2/rke2.yaml ."
#   }
#   provisioner "local-exec" {
#     command = "nh=\"https://${local.ssh_ip}:6443\"; yq -y --arg nh \"$nh\" '.clusters[0].cluster.server = $nh' rke2.yaml > kubeconfig.yaml"
#   }
#   provisioner "local-exec" {
#     command = "install -d ${pathexpand("~/.config/harvester")}"
#   }
#   provisioner "local-exec" {
#     command = "cp kubeconfig.yaml ${pathexpand("~/.config/harvester/kubeconfig.yaml")}"
#   }
# }
